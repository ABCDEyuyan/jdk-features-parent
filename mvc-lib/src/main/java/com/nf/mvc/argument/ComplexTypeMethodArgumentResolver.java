...
public class ComplexTypeMethodArgumentResolver implements MethodArgumentResolver {
     ...
     * @throws Exception
     */
    private Object invokeSetterMethod(Object instance, Method method, String
            parameterName,HttpServletRequest request, Stack<String> prefixStack) throws Exception {
        //处理参数的前缀，如果需要加前缀的话，就把参数名改掉，添加上前缀
        parameterName = handleParameterNamePrefix(prefixStack, parameterName);
    ...
    private String handleParameterNamePrefix(Stack<String> prefixStack, String parameterName) {
 ...
 * </ol>
 * 强烈建议：此解析器应该是整个解析器链的最后一个解析器</p>
 * <p>此类也利用了{@link com.nf.mvc.argument.HandlerMethodArgumentResolverComposite}类进行了自定义的解析器组合，
 * 先利用这个解析器组合进行解析，解析不了就交给本类解析</p>
 * <p>
 *     这里也演示了多线程环境下单例模式的使用，以避免重复的创建自定义解析器组合。具体见{@link #getResolvers()},
 *     需要注意的是本类也是单例模式下运行的
