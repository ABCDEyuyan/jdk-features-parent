 ...
 *         <li>有了此类的实例之后，可以再调用addResolver与insertResolver之类的方法再进行解析器的组合</li>
 *     </ul>
 * 典型的组合解析器的代码如下：
 *     <pre class="code">
 ...
 *     </pre>
 * Mvc框架中的参数解析器都是在servlet初始化阶段实例化的，并且是单例的，但此解析器并不要求是单例的，
 * 且任何时候都可以进行实例化的操作，并不限定在初始化阶段
 * </p>
 * <h3>清空组合里的解析器</h3>
 * <p>如果要清空此类的所有的解析器组合,可以调用方法{@link #clear()}</p>
 * <h3>使用场景</h3>
 * <p>所有需要进行方法参数解析的类，都可以直接借助此类实现参数的解析，比如mvc框架中的HandlerAdapter实现
 *     ExceptionResolver以及BeanMethodArgumentResolver的实现，不同的类型可以借助此类组合出自己所需要的一套参数解析器集合，
 *     比如HandlerAdapter可以利用此类组合几个解析器来解析参数，而异常解析器也可以自己组合另一套解析器供自己使用
 * </p>
 * <h3>使用方法</h3>
 * <p>使用此类与使用某个具体的解析器有一点不同，它不需要先调用{@link #supports(MethodParameter)}方法，
 * 可以直接调用{@link #resolveArgument(MethodParameter, HttpServletRequest)}来进行参数解析</p>
 * <h3>缓存</h3>
 * <p>由于mvc框架的参数解析器主要是基于类型的，所以不管是那个方法的参数，只要参数的类型一样，其解析器都会是同一个。
 * 为了提高性能，这里用了缓存，只要找到某一个类型的缓存就保存到缓存中，下次再来查找时就直接从缓存中提取，
 * 具体见{@link #getArgumentResolver(MethodParameter)}实现，所以这种缓存容器是渐进式增长的，
 * 这一点与{@link WebTypeConverters#getTypeConverter(Class)} }是不一样的，后者类加载时已经全部缓存起来了，容量不会变化了。
 * 也与{@link com.nf.mvc.mapping.RequestMappingHandlerMapping#getHandler(HttpServletRequest)}
 * 实现的缓存逻辑不太一样，后者容量超过设定的阈值后会清理掉超量的访问频率低的缓存项，此类永不进行缓存的清理操作</p>
 *
...
public class MethodArgumentResolverComposite implements MethodArgumentResolver {
    ...
    public Object resolveArgument(MethodParameter parameter, HttpServletRequest request) throws Exception {
        ...
        if (resolver == null) {
            throw new IllegalArgumentException("不支持的参数类型 [" + parameter.getParameterType()
                    + "]. 当前解析的参数名是:[" + parameter.getParameterName()
                    + "],所在的方法是:[" + parameter.getMethod().getName()
                    + "]，所在的类是:[" + parameter.getContainingClass().getName() + "]");
        }
