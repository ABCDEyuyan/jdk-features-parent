 ...
 * 只有参数解析器会基于某些规则对null值进行一些特别的处理,比如从{@link com.nf.mvc.argument.RequestParam}中获取默认值</p>
 * <p>在设计时你可以不把类设计为泛型类,而只是把方法设计为泛型方法,比如{@code <T> T convert(String paramValue)},但这样设计的话,
 * 在实现类编写实现方法时,比如:{@code BigDecimal convert(String paramValue}会报一个警告:"需要的是T,但此刻是BigDecimal,建议把类泛型化",
 * 这样子类在实现时，在类级别就把T实参化，这样就不会报警告了，符合java泛型的规则</p>
 *
