...
- 在配置类中通过一些Bean方法来注册组件，比如defaultServletHandlerMapping方法来注册一个HandlerMapping
  ...
  - 调用特定类的配置类的某个方法用来得到配置的最终结果，比如调用DefaultServletHandlerConfigurer 的buildHandlerMapping方法得到一个SimpleUrlHandlerMapping对象的bean的注册
...
- 给HandlerMethod添加Object  instance这样的属性（ok）
- 实现RequestMappingHandlerAdapter（ok）
...
- FileViewResult编写（ok）
- 把handler执行结果适配为ViewResult，（ok）
- RequestBody的反序列化（ok）
- configurer ：equal，equalIgnoreCase (PathMatcher)
- HandlerExecutionChain(ok)
- argumentResolver composite(cache)
- exceptionresolver （ExceptionHandler注解要不要支持能处理的异常数组）？ 获取真正的异常
- multipart 文件名解析完善
- cors ()
- HandlerContext(ok)
- AnnotationUtils？
- Servlet API argument Resolver (ok)
- IDEA json 文本输入问题
- 把复杂类型参数解析器改成自己写的MethodInvoker
- javassist参数名解析bug （ok)
...
https://zhuanlan.zhihu.com/p/142667371#:~:text=Caffeine%20%E4%B8%BA%E6%88%91%E4%BB%AC%E6%8F%90%E4%BE%9B%E4%BA%86,%E4%B8%89%E7%A7%8D%E8%BF%87%E6%9C%9F%E7%AD%96%E7%95%A5%20%EF%BC%8C%E5%88%86%E5%88%AB%E6%98%AF%E5%9F%BA%E4%BA%8E%E5%A4%A7%E5%B0%8F%EF%BC%88size-based%EF%BC%89%E3%80%81%E5%9F%BA%E4%BA%8E%E6%97%B6%E9%97%B4%EF%BC%88time-based%EF%BC%89%E3%80%81%E5%9F%BA%E4%BA%8E%E5%BC%95%E7%94%A8%EF%BC%88reference-based%EF%BC%89
...
https://zhuanlan.zhihu.com/p/347342971 只读这篇文章就够了，重点是：对象放置到散列表中时，先调用hashcode方法确定此hashcode值对应的存储位置有没有数据，没有就直接存，有就再调用equals进行判等，如果相等就表示有重复数据，如果是把对象放到不允许重复的集合类型，比如HashSet，那么就不放入这个对象
